...
namespace Transport.Net
{
    ...
    public class MachineReader
    {
        ...
        public static List<IMachine<string>> ReadMachines(string machineSection = "Machine")
        {
            ...
            var root = configuration.GetSection("Transport.Net").GetSection(machineSection).GetChildren();
            foreach (var machine in root)
            {
                ...
                foreach (var param in kv)
                {
                    switch (param.Key)
                    {
                        case "protocol":
                            {
                                paramsSet.Add(Enum.Parse(Assembly.Load("Transport.Net." + dic["protocol"]).GetType("Transport.Net." + dic["protocol"] + "." + dic["protocol"] + "Type"), dic["type"]));
                                break;
                            }
                        case "type":
                            {
                        ...
                        case "model":
                            {
                                paramsSet.Add(Enum.Parse(Assembly.Load("Transport.Net." + dic["protocol"]).GetType("Transport.Net." + dic["protocol"] + "." + dic["protocol"] + "MachineModel"), dic["model"]));
                                break;
                            }
                        case "addressMap":
                            {
                                var machineTypeTemp = Assembly.Load("Transport.Net." + dic["protocol"]).GetType("Transport.Net." + dic["protocol"] + "." + dic["protocol"] + "Machine`2");
                ...
                Type machineType = Assembly.Load("Transport.Net." + dic["protocol"]).GetType("Transport.Net." + dic["protocol"] + "." + dic["protocol"] + "Machine`2");
    ...
    internal class AddressReader<TUnitKey, TAddressKey, TSubAddressKey> where TUnitKey : IEquatable<TUnitKey> where TAddressKey : IEquatable<TAddressKey> where TSubAddressKey : IEquatable<TSubAddressKey>
    {
        ...
        public static IEnumerable<AddressUnit<TUnitKey, TAddressKey, TSubAddressKey>> ReadAddresses(string addressMapName)
        {
            ...
            var addressesRoot = configuration.GetSection("Transport.Net").GetSection("AddressMap").GetSection(addressMapName).GetChildren();
