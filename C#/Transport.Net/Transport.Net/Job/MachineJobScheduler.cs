...
namespace Transport.Net
{
        ...
        public static async Task<MachineGetJobScheduler<TMachineMethod, TMachineKey, TReturnUnit>> CreateSchedulerMillisecond(string triggerKey, int count = 0, int intervalMilliSecond = 1000)
        {
            ...
            if (intervalMilliSecond <= 0)
            {
                trigger = TriggerBuilder.Create()
                    .WithIdentity(triggerKey, "Transport.Net.DataQuery.Group." + triggerKey)
                    .StartNow()
                    .Build();
            }
            else if (count >= 0)
            ...
            if (intervalMilliSecond <= 0)
            {
                listener = new JobChainingJobLIstenerWithDataMapRepeated("Transport.Net.DataQuery.Chain." + triggerKey, new string[2] { "Value", "SetValue" }, count);
            }
            else
            {
                listener = new JobChainingJobListenerWithDataMap("Transport.Net.DataQuery.Chain." + triggerKey, new string[2] { "Value", "SetValue" });
            }
            scheduler.ListenerManager.AddJobListener(listener, GroupMatcher<JobKey>.GroupEquals("Transport.Net.DataQuery.Group." + triggerKey));

            if (await scheduler.GetTrigger(new TriggerKey(triggerKey)) != null)
            {
                await scheduler.UnscheduleJob(new TriggerKey(triggerKey, "Transport.Net.DataQuery.Group." + triggerKey));
            }
            var jobKeys = await scheduler.GetJobKeys(GroupMatcher<JobKey>.GroupEquals("Transport.Net.DataQuery.Group." + triggerKey));
    ...
    public sealed class MachineGetJobScheduler<TMachineMethod, TMachineKey, TReturnUnit> where TMachineKey : IEquatable<TMachineKey> where TReturnUnit : struct where TMachineMethod : class, IMachineMethod
    {
        ...
        {
            JobKey jobKey = JobKey.Create("Transport.Net.DataQuery.Job." + queryId, "Transport.Net.DataQuery.Group." + _trigger.Key.Name);
            ...
            {
                var listener = _scheduler.ListenerManager.GetJobListener("Transport.Net.DataQuery.Chain." + _trigger.Key.Name) as JobChainingJobListenerWithDataMap;
                if (listener == null) throw new NullReferenceException("Listener " + "Transport.Net.DataQuery.Chain." + _trigger.Key.Name + " is null");
