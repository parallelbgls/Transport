Transport.Net
===================
[![NuGet](https://img.shields.io/nuget/v/Transport.Net.svg)](https://www.nuget.org/packages/Transport.Net/)
...
BaseUtility is an abstract class. You can check all apis in BaseUtility.cs in Transport.Net project.
...
The main target of Transport.Net is building a high extensable hardware communication protocol, so we allow everyone to extend the protocol.

To extend Transport.Net, first of all ValueHelper.cs in Transport.Net is a really powerful tool that you can use to modify values in byte array.There are three ValueHelpers: LittleEndianLsbValueHelper(Little Endian), BigEndianLsbValueHelper(Big Endian) and BigEndianMsbValueHelper(Big Endian with bit reverse). Remember using the correct one.<br>
If you want to write a new one, just write in namespace "Transport.Net" in assmenly start with "Transport.Net." and Transport.Net will automatically load it.

In this tutorial I will use Transport.Net.Modbus to tell you how to implement your own protocol.
...
If you want to connect to hardware using another way, please implement your own ProtocolLinker and BaseConnector. And please remember that if you want to connector hardware using serial line, please don't use ComConnector in Transport.Net and implement your own ComConnector.
...
```
First of all, there are two types of coordinates in Transport.Net Address System - Protocol Coordinate and Abstract Coordinate.

Here is an example of the differences between them:

In Register of Modbus, the minimum type is short, but Transport.Net use type of byte to show the result. If you want to get value from 400003 in protocol coordinate, you actually get 5th and 6th byte value in abstract coordinate.
...
For example if you want the get a value from the 6th byte in Hold Register. In traditional modbus you can only get 400003 of 2 bytes and get the 2nd byte from it. But in Transport.Net there is an easy way to get it.
```
...
```Json
{
    "Transport.Net:TCP:192.168.1.100:502:FetchSleepTime": 50,
    "Transport.Net:TCP:192.168.1.101:FetchSleepTime": 50,
    "Transport.Net:COM:COM1:FetchSleepTime": 2000
}
```
